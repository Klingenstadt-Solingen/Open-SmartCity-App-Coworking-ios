//
//  OSCACoworkingFormContactTests.swift
//  OSCACoworkingTests
//
...
///  | pygmentize -g
///  ```
class OSCACoworkingFormContactTests: OSCAParseClassObjectTestSuite<OSCACoworkingFormContact> {
  override open func makeSpecificObject() -> OSCACoworkingFormContact? {
    nil
  ...
  /// The file name has to match with the test class name: `OSCACoworkingFormContactTests.json`
  override func testJSONDataAvailable() throws -> Void {
    try super.testJSONDataAvailable()
  }// end override func testJSONDataAvailable
#if DEBUG
  /// Is it possible to deserialize `JSON` scheme example data to an array  `OSCACoworkingFormContact` 's?
  override func testDecodingJSONData() throws -> Void {
    try super.testDecodingJSONData()
  }// end override func testDecodingJSONData
#endif
  /// is it possible to fetch up to 1000 `OSCACoworkingFormContact` objects in an array from network?
...
}// end class OSCACoworkingFormContactTests
#endif
